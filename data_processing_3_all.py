...
class DataProcessor:
    def load_and_prepare_data(self):
        ...
        calculation_columns = ['Номер ТС', 'Исполнитель',
                               ...
                               'Стоимость перевозки факт (Экономика)']
        df = df[calculation_columns]
        def adjust_volumes(row):
            if row['Фактический объем кузова, м3'] < row['Суммарный объем грузовых ед.,м3']:
                row['Фактический объем кузова, м3'] = row['Суммарный объем грузовых ед.,м3']
            else:
                return row
            return row
...
# Применение функции к каждому ряду
        ...
        df= df.apply(adjust_volumes, axis=1)
...
# Вывод количества строк
    ...
    def prepare_data(self, df, ufps, data_transport, naym, all_car, calculation, parametr):
        ...
        df['затраты на км'] = df[cost_col] / df['Расстояние по маршруту Факт,км']
        ...
        if naym == 1:
            def get_cars_pension():
                ...
                df = xl.parse(sheet_name, skiprows=1)
                col = ['Номер ТС', weight_col, cost_col, 'Маршрут', 'Расстояние по маршруту Факт,км',
        ...
        dpt_target_M_d_kg = pd.pivot_table(df_target, index='Маршрут', values=[sum_col, cost_col], aggfunc='sum')
        ...
        if naym == 1:
            def get_target_pension(ufps, data_transport):
                ...
                col_table_target_moscow_1day = ['Маршрут', 'Расстояние по маршруту Факт,км', cost_col, sum_col]
...
if __name__ == "__main__":
    ...
    calculation ='volume' #''  # or volume
    parametr ='TMS'  #or  economic
    df = processor.load_and_prepare_data()
    table_cars, table_target, df_combined = processor.prepare_data(df, ufps, data_transport, naym=1, all_car=1, calculation=calculation,parametr=parametr)
    ...
    all=df_cleaned.shape[0]
    filtered_df = df_cleaned[df_cleaned['Фактический объем кузова, м3'] < df_cleaned['Суммарный объем грузовых ед.,м3']]
