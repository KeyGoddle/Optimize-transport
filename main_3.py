...
class MainProcessor:
    ...
    def run_main(self, ufps, data_transport, calculation,parametr,naym,all_car):
        ...
        if naym == 1:
            ...
            if calculation =='volume':
                table_cars_moscow.loc[table_cars_moscow['Модель ТС'].str.contains('Наемник'), 'затраты на км'] += 400
    ...
    def adjust_costs_for_naym(self, table_cars_moscow):
        ...
        table_cars_moscow.at[index, 'затраты на км'] = ((prev_cost + true_cost) / 2) + 150

        index = table_cars_moscow.index[table_cars_moscow['Модель ТС'] == 'Наемник - 1500'][0]
        true_cost = table_cars_moscow.at[index, 'затраты на км']
        table_cars_moscow.at[index, 'затраты на км'] = true_cost + 141

        index = table_cars_moscow.index[table_cars_moscow['Модель ТС'] == 'Наемник - 2000'][0]
        true_cost = table_cars_moscow.at[index, 'затраты на км']
        table_cars_moscow.at[index, 'затраты на км'] = true_cost + 131

        index = table_cars_moscow.index[table_cars_moscow['Модель ТС'] == 'Наемник - 5000'][0]
        true_cost = table_cars_moscow.at[index, 'затраты на км']
        table_cars_moscow.at[index, 'затраты на км'] = true_cost + 136
def get_all_data():
    data_processor = DataProcessor()
    df = data_processor.load_and_prepare_data()
...
if __name__ == "__main__":
    ...
    table_target_results_by_week=pd.DataFrame(columns=['Маршрут','Расстояние по маршруту Факт,км',	cost_col,	sum_col	,'Количество рейсов',	'Модель ТС',	'Стоимость'	,'УФПС',	'Даты начала перевозки по маршруту'])



    df_cars_compare=pd.DataFrame(columns=['Модель ТС','Количество используемых машин (Факт)','Количество используемых машин (Оптимизация)','Даты начала перевозки по маршруту']) 
    ...
    for data in all_data:
        ...
        grouped_df[f'{sum_col} (ФАКТ)']=df_cleaned[sum_col].sum()
        grouped_df[f'{sum_col}  (Оптимизация)']=table_target_moscow_1day_results[sum_col].sum()
        ...
        df_cleaned['Количество используемых машин'] = 1
        df_cleaned['Даты начала перевозки по маршруту'] = (df_cleaned['Даты начала перевозки по маршруту']).dt.date
        if naym==1:
            df_cleaned.loc[df_cleaned['Модель ТС'] == 'ТЭК', 'Модель ТС'] = df_cleaned.loc[df_cleaned['Модель ТС'] == 'ТЭК', weight_col].apply(lambda row: 'Наемник - ' + str(round(row)))
        table_target_moscow_1day_results['Модель ТС'] = table_target_moscow_1day_results['Модель ТС'].apply(lambda x: x[0] if isinstance(x, list) and len(x) > 0 else x)
        df_fact_cars = df_cleaned.groupby(['Даты начала перевозки по маршруту', 'Модель ТС']).agg({'Количество используемых машин': 'sum'}).reset_index()
        print(df_fact_cars)

        table_target_moscow_1day_results['Даты начала перевозки по маршруту'] = (table_target_moscow_1day_results['Даты начала перевозки по маршруту']).dt.date
        df_optimization_cars = table_target_moscow_1day_results.groupby(['Даты начала перевозки по маршруту', 'Модель ТС']).agg({'Количество рейсов': 'sum'}).reset_index()
        df_cars_compare_vrem= pd.merge(df_fact_cars,df_optimization_cars,on=['Даты начала перевозки по маршруту', 'Модель ТС'],suffixes=(' (Факт)', ' (Оптимизация)'),how='outer')
        df_cars_compare_vrem.rename(columns={'Количество используемых машин': 'Количество используемых машин (Факт)','Количество рейсов': 'Количество используемых машин (Оптимизация)'}, inplace=True)
        df_cars_compare = pd.concat([df_cars_compare,df_cars_compare_vrem], ignore_index=True)
        print(df_cars_compare)
    ...
    print(df_cars_compare)
    df_cars_compare.to_excel('df_cars_compare.xlsx')